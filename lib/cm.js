...
import { EditorState, Compartment, Prec, RangeSetBuilder, StateEffect, StateField } from '@codemirror/state';
...
const clearAllMarksEffect = StateEffect.define();
const removeMarksEffect = StateEffect.define();

function buildDecorations(marks) {
...
function offsetToDocPos(view, offset) {
  const line = view.state.doc.lineAt(offset);
  return { line: line.number - 1, ch: offset - line.from };
}

// ── Editor adapter (CM5-compatible surface over a CM6 EditorView) ──
export function createEditor({ parent, initialValue, onChange, themeName, keymapBindings }) {
  const themeCompartment = new Compartment();
  const historyCompartment = new Compartment();

  const setTheme = name => getTheme(name);
  ...
  return {
    ...
    getLine: n => view.state.doc.line(n + 1).text,
    replaceRange: (text, from, to) =>
      view.dispatch({
        changes: {
          from: toOffset(view, from),
          to: to ? toOffset(view, to) : undefined,
          insert: text,
        },
      }),
    ...
    posFromIndex: i => i,
    indexFromPos: pos => toOffset(view, pos),
    focus: () => view.focus(),
    ...
    setOption: (_name, value) => view.dispatch({ effects: themeCompartment.reconfigure(setTheme(value)) }),
