export function withTimeout(promise, ms, onTimeout, message = 'Timed out') {
  return Promise.race([
    promise,
...
export function pickBestCachedRecord(records) {
  let best = null;
  let bestTs = -1;
...
export async function listCachedNotePaths() {
  try {
    ...
    return result.filter(k => typeof k === 'string' && k.startsWith('memoweb_cache:'));
  } catch (e) {
