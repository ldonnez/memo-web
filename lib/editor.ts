...
import type { CmAdapter, Note } from './types.ts'

export const PASS = Symbol('pass')

export interface EditorContext {
  formatTable: (c: CmAdapter, insertLine: number, onEditorInput: () => void) => void
  onEditorInput: () => void
}

export function smartEnter(c: CmAdapter, { formatTable, onEditorInput }: EditorContext): symbol | void {
  const cursor = c.getCursor()
  const line = c.getLine(cursor.line)
  if (line[0] === '|' && line.split('|').length >= 4 && cursor.ch >= line.indexOf('|')) {
    formatTable(c, cursor.line + 1, onEditorInput)
    return
  }
  let m = line.match(/^(\s*(?:[-*+]|\d+[.)])\s+(?:\[[ x]\]\s+)?)/)
  if (!m || cursor.ch < m[1]!.length) return PASS
...
export function toggleTaskByIndex(idx: number, cm: CmAdapter | null, onEditorInput: () => void): void {
  const content = cm
    ? cm.getValue()
...
export function formatTable(c: CmAdapter, insertLine: number, onEditorInput: () => void): void {
...
export function moveInTable(c: CmAdapter, shift: boolean): boolean {
...
export function handleTab(c: CmAdapter): void {
  if (moveInTable(c, false)) return
  c.execCommand('insertSoftTab')
}

export function handleShiftTab(c: CmAdapter): symbol | void {
  if (moveInTable(c, true)) return
  return PASS
}

export function toggleTaskOnLine(cm: CmAdapter | null, currentFile: Note | null, onEditorInput: () => void): void {
...
export function insertMarkdown(before: string, after: string, cm: CmAdapter | null, onEditorInput: () => void): void {
  if (cm) {
...
export function insertTimestamp(cm: CmAdapter | null, onEditorInput: () => void): void {
  const now = new Date()
