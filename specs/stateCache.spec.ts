...
function mapEntry(item: ListingItem, contentCache: Map<string, string>, draftCache: Map<string, string>): Note {
  return {
    name: item.name,
    path: item.path,
    sha: item.sha,
...
function simulateFetchAllNotesContent(
  notes: Note[],
  contentCache: Map<string, string>,
  fakeContent: (path: string) => { content: string; sha: string } | null,
): Note[] {
  return notes.map(n => {
...
function makeListingEntry(name: string, overrides: Partial<ListingItem> = {}): ListingItem {
  return {
    name,
    path: name,
    type: 'file',
...
describe('contentCache — note mapping after save', () => {
  it('sets dirty=false and populates content after save via markNoteClean + cache update', () => {
    ...
    const fakeContent = (_p: string) => ({ content: Buffer.from('original').toString('base64'), sha: 'sha1' })
    ...
    assert.strictEqual(savedNote.content, Buffer.from('edited text').toString('base64'), 'note.content updated in list')
  ...
  it('retains contentCache entry after navigating to another dir and back', () => {
    ...
    const fakeContent = (p: string) => ({ content: Buffer.from('content-' + p).toString('base64'), sha: 's1' })
...
describe('contentCache — fetchAllNotesContent skip behavior', () => {
  it('skips notes that already have content in the list', () => {
    const cc = new Map<string, string>()
    const notes = [
      makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: 'existing-b64', sha: 'olds', dirty: false }),
      makeNote({ name: 'b.md.gpg', path: 'b.md.gpg', content: null, sha: 'sha-b', dirty: false }),
    ]
    // Mark a as already cached
    cc.set('a.md.gpg', 'existing-b64')

    const called: string[] = []
    const fakeContent = (p: string) => {
      called.push(p)
  ...
  it('populates contentCache for newly fetched notes', () => {
    const cc = new Map<string, string>()
    const notes = [makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: null, sha: 'sha-a', dirty: false })]
    const fakeContent = (_p: string) => ({ content: 'cGF5bG9hZA==', sha: 'sha-new' })
    const updated = simulateFetchAllNotesContent(notes, cc, fakeContent)
  ...
  it('does not throw when ghGetFile fails for a note', () => {
    const cc = new Map<string, string>()
    const notes = [makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: null, sha: 'sha-a', dirty: false })]
    const fakeContent = (_p: string) => null // ghGetFile returned null/undefined
    const updated = simulateFetchAllNotesContent(notes, cc, fakeContent)
...
describe('contentCache — offline fallback', () => {
  it('populates contentCache from cached note data on offline load', () => {
    const cc = new Map<string, string>()
    const cachedNotes = [
      makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: 'cached-b64', sha: 'cached-sha' }),
      makeNote({ name: 'b.md.gpg', path: 'b.md.gpg', content: null, sha: 'sha-b' }),
    ]
  ...
  it('offline-load re-syncs dirty flag from draftCache even when cached note is clean', () => {
    ...
    const cached = [makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: 'b64', sha: 's', dirty: false })]
    ...
    assert.strictEqual(formatNoteItem(notes[0]!, undefined).includes('status-badge dirty'), true)
  })

  it('offline-load keeps dirty=false when no draft exists for the note', () => {
    const dc = new Map<string, string>()
    const cached = [
      makeNote({ name: 'a.md.gpg', path: 'a.md.gpg', content: 'b64', sha: 's', dirty: false }),
      makeNote({ name: 'b.md.gpg', path: 'b.md.gpg', content: 'b64', sha: 's', dirty: false }),
    ]
...
describe('multiple notes — independent cache state', () => {
  it('saving one note does not affect another notes cached content', () => {
    ...
    const fakeContent = (p: string) => ({ content: Buffer.from(p + '-content').toString('base64'), sha: 's-' + p })
...
describe('edge cases — contentCache boundary', () => {
  ...
  it('contentCache with non-string value is passed through as-is', () => {
    const cc = new Map<string, string>()
    cc.set('a.md.gpg', null as unknown as string)
    const dc = new Map<string, string>()
    const entries = [makeListingEntry('a.md.gpg')]
    const notes = mapEntries(entries, cc, dc)
...
describe('edge cases — draftCache boundary', () => {
  ...
  it('draft with empty string still marks note as dirty', () => {
    const cc = new Map<string, string>()
    const dc = new Map<string, string>([['a.md.gpg', '']])
    const entries = [makeListingEntry('a.md.gpg')]
    const notes = mapEntries(entries, cc, dc)
  ...
  it('draftCache entries for non-existent listing entries are ignored', () => {
    const cc = new Map<string, string>()
    const dc = new Map<string, string>([['ghost.md.gpg', 'ghost draft']])
    const entries = [makeListingEntry('a.md.gpg')]
    const notes = mapEntries(entries, cc, dc)
