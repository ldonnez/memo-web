...
import { formatNoteItem } from '../lib/util.ts'
...
describe('formatNoteItem', () => {
  it('renders a clean note without asterisk or badge', () => {
    const note = makeNote()
    const html = formatNoteItem(note, undefined)
  ...
  it('escapes HTML in dirty asterisk position is correct', () => {
    const clean = formatNoteItem(makeNote({ dirty: false }), undefined)
    const dirty = formatNoteItem(makeNote({ dirty: true }), undefined)
  ...
  it('dirty note has no asterisk when name ends with gpg', () => {
    ...
    const dirty = formatNoteItem(makeNote({ name: 'note.md.gpg', dirty: true }), undefined)
